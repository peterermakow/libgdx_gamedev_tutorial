...
Реализуем управляемый космический корабль(далее КК), который будет обладать какой-то инерцией при движении. 

**Как сделать ускорение и замедление космического корабля**
Допустим у нас есть вектор местоположения объекта (10,4) и вектор скорости (1,1), при их сложении получается движение объекта
Если же на каждом шаге увеличивать вектор скорости например на 0.1, скорость будет расти

Создадим класс **Hero**. У КК будет *position*, скорость *velocity*, мощность двигателя *enginePower*, а также угол *angle*, показывающий куда направлен КК. Позже прикрутим другие поля, в том числе касающиеся стрельбы. 
