...
Изначально главный класс игры наслудется от *ApplicationAdapter*, в последствии мы поменяем чтобы он наследовался от класса Game. Итак, в настоязий момент в классе StarGame нам необходимы методы **create** (он запускается при старте приложения),
метод **render** отрабатывает 60 раз в секунду, а **dispose** при закрытии программы очищает ресурсы. Так, LibGDX использует GPU, а Garbage Collectors не умеют чистить видеопамять.
